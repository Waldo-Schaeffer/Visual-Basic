...
'--------进程句柄全局函数--------
Public Function GetProcessHwnd(Pid As Long) As Long
...
'--------进程标识全局函数--------
Public Function GetProcessID(ProcessName As String, Optional CaseSensitive As Boolean = False) As String
  ...
  Do While ProcessValue
    If CaseSensitive = True Then
      If ProcessName = Left(ProcessObject.szExeFile, InStr(ProcessObject.szExeFile, Chr(0)) - 1) Then
        If PidStr <> "" Then
          PidStr = PidStr & "," & CStr(ProcessObject.th32ProcessID)
        Else
          PidStr = CStr(ProcessObject.th32ProcessID)
        End If
      End If
    Else
...
'--------进程名称全局函数--------
Public Function GetProcessName(Pid As Long) As String
...
'--------进程路径全局函数--------
Public Function GetProcessPath(Pid As Long) As String
...
For Each WMI In GetObject("winmgmts:\\.\root\cimv2:win32_process").instances_
  If WMI.ProcessID = Pid Then
    GetProcessPath = WMI.executablepath
    Exit For
  End If
Next
